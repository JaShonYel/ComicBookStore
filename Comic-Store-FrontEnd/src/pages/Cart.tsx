            ...
            <div className="cart-items-container flex-fill" style={{ minWidth: "300px" }}>
              {cartItems.map(item => (
                <div
                  ...
                  style={{ border: "2px solid #000", borderRadius: "8px", backgroundColor: "#fff" }}
                >
                  <div className="d-flex align-items-center">
                    <Image src={item.img} alt={item.title} className="cart-thumbnail me-3" />
                    <span className="fw-bold">{item.title}</span>
                  </div>
                  <div className="d-flex flex-column align-items-end">
                    <span className="mb-2">${Number(item.price).toFixed(2)}</span>
                    <Button
